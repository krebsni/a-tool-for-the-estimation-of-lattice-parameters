#!/usr/bin sage
r"""
A main script of the ``lattice_parameter_estimation`` library to find secure parameter sets of popular versions of lattice problems (LWE and SIS in their integer, ring, and module versions) given various :math:`\ell_p`-norms.
This script assumes that the LWE estimator commit fb7deba in a subdirectory `estimator`.
...
def generic_search(
    ...
    r"""Generic search for a parameter set such that all problem instances generated by ``parameter_problem`` for the given paramter set satisfy a given bit security level.

    The search starts with the parameter set ``initial_parameters``. At the end of each iteration, new parameter sets are generated using ``next_parameters`` and sorted into an ordered list of parameter sets according to ``parameter_cost``. If no ``parameter_cost``-function is specified, :func:`param_search.unit_cost` is used. In an iteration, the first parameter set in the list is retrieved and a set of problems instances is generated for the given parameter set according to ``parameter_problem`` and a cost estimation algorithm is executed on all generated problem instances. If one of the cost estimation algorithms specified in ``config`` returns a cost lower than :math:`2^\text{sec}`, the cost estimation for the given parameter set is terminated early. If all algorithms return cost higher than :math:`2^\text{sec}`, the problem instances in the set are secure and the current parameter set is returned in addition to the aggregate cost results.
    ...
    :param config: configuration, instance of :py:class:`algorithms.Configuration`
    :param scalar_parameters: True if parameter sets are scalars, else parameter sets must be tuples.

    :returns: dict with entries ``"parameters"`` (secure paramter set) and ``"result"`` (instance of :py:class:`problem.AggregateEstimationResult` containing cost estimates and results from all estimation algorithms).
    """
    start = time.time()
