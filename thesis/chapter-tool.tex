...
\begin{theorem}\label{th:norm-rel2}
    ...
    \end{align}
\end{theorem}
We can thus bound a given bound in the norm $\ell_q$ for some $q$ by a bound in some $\ell_p$ norm using the above inequalities and analogously in the embedding norm, which we call $\mathcal{C}_\infty$. We combine both results in the method $\texttt{to\_lp()}$ in the norm classes $\texttt{Lp}$ and $\texttt{Coo}$.

% From the above inequations, we obtain the following theorems that we use in the method $\texttt{to\_lp()}$.
...
%               \end{align}
%               From \cref{eq:norm-1-leq-inf} and with $2\leq p < \infty$ it follows that
...
%                   \end{aligned}
%               \end{equation}
%         \item From \cref{eq:norm-1-leq-inf,eq:norm-inf-leq-1} and with $p\geq 2$ it follows that
%               \begin{equation}\label{norm1}
%                   \begin{aligned}
%                       \| f \|_p & \leq  \| f \|_1,                          \\
%                       \| f \|_p & \leq  \| f \|_2,                          \\
%                       \| f \|_p & \leq n^{\frac{1}{p} - 0}  \| f \|_\infty.
%                   \end{aligned}
%               \end{equation}
%         \item From \cref{eq:norm-inf-leq-1} it follows that
%               \begin{align*}
...
If the two norms bounds that we want to multiply are given in a different norm, we simply convert the bounds to the $\mathcal{C}_\infty$ norm as described in \cref{eq:Coo-norm}.
