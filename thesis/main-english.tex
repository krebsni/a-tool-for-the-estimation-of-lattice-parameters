...
- (shifted) fundamental parallelipiped \label{eq:fundamental-parallelipiped}: $\mathcal{P}_{1/2}(\mathbf{B}) = \mathbf{B} \cdot \left[ - \frac{1}{2}, \frac{1}{2}\right)^n = \left\{ \mathbf{x} \in \mathbb{R}^m \mid \mathbf{x} \sum_{i=1}^n \gamma_i \mathbf{x}_i, \gamma_i \in  \left[ - \frac{1}{2}, \frac{1}{2}\right) \right\}$ every coset has representative  % c - B * \left\lfloor^{-1} \cdot c\right\rceil
...
- with high probability determinant of $q$-ary lattice is $\det(\Lambda_q(\mathbf{A}))=q^{m-n}$ if $\mathbf{A} \in \mathbb{Z}_q^{m\times n}$ % TODO insert proof from chapter-algorithm \cref{eq:det-MR}


* Gram-Schmidt basis \label{sec:gram-schmidt}% TODO: maybe move to lattice reduction?
