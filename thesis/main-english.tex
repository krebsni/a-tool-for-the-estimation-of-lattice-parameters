...
An important concept is the \textit{fundamental domain} of a lattice. The fundamental domain is a subset of $\mathbb{R}^m$ that contains exactly one representative of every coset of $\mathbf{R}^m/\mathbf{\Lambda}$. The most commonly considered fundamental domain of a lattice with basis $\mathbf{B}$ is the (shifted) \textit{fundamental parallelipiped}
...
\end{equation}
By applying Stirling's approximation as described in \cite{Gop16}, we can simply the estimate to
...
\end{equation}
$\Lambda_q(\mathbf{A}^\intercal)$ is commonly referred to as the \textit{LWE lattice} as finding a short vector in $\Lambda_q(\mathbf{A}^\intercal)$ corresponds to solving LWE. The second, referred to as the \textit{(dual) SIS lattice}, is given by
...
\subsection{Linear Code} \label{sec:linear-code} % TODO maybe move to Coded BKW, maybe too much...
...
In the previous section, we already mentioned the Shortest Vector Problem or short SVP. In this section, we want to briefly list a number of lattice problems used in this work. The definitions are based on \cite{MG02} and \cite{LM09}.
...
The corresponding decision version is the \textsc{GapSVP}$_\gamma$ problem in which we are asked to decide whether $\lambda_1(\Lambda) \leq d$ or $\lambda_1(\Lambda) \geq \gamma d$ given a basis $\mathbf{B}$ of $\Lambda$ and a real number $d$. If neither is the case, any answer is correct \cite{LM09}.
...
\begin{definition}[HSVP$_\kappa$] \label{def:hsvp}
  Given a basis $\mathbf{B}$ of a lattice $\Lambda(\mathbf{B}) \in \mathbb{R}^m$, the (approximate)  Hermite Shortest Vector Problem (HSVP$_\kappa$) is the problem of finding a nonzero lattice vector $\mathbf{v} \in \Lambda$ such that $\| \mathbf{v} \| \leq \kappa \cdot \text{det}(\Lambda)^{\frac{1}{n}}$.
\end{definition}
...
\end{definition}
A discrete Gaussian sampler over a lattice can be efficiently realized (see \cite{GPV08}). In order to avoid confusion, throughout this work we use $\sigma$ to denote the standard deviation, where $\sigma = \frac{s}{\sqrt{2 \pi}}$, and define $\alpha = \frac{s}{q} = \frac{\sqrt{2\pi} \sigma}{q}$.
...
In \cref{sec:intro} we informally introduced two important problems, namely, the Learning with Errors and the Short Integer Solution problems, that constitute the main focus of this work. We will now describe them in more detail.
...
Additions are performed in $\mathbb{Z}_q$. In the case that $q=2$, LWE corresponds to the \textit{Learning Parity with Noise} (LPN) problem. % TODO still needed with Search-LWE\ldots???
...
\end{equation}
with generator matrix $\mathbf{A}$ for a linear code over $\mathbb{Z}_q$ and $\mathbf{z}$ as the received word (see \cref{sec:linear-code}). Finding the secret vector $\mathbf{z}$ is equivalent to finding the codeword $\mathbf{y} = \mathbf{A}^\intercal\mathbf{s}$ with minimum distance $\| \mathbf{y} - \mathbf{z} \|$.
