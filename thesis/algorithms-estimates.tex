...
enumerating all lattice vectors within a bounded region % TODO how? maybe check out ABKFSW20 2.2
...
most efficient: Kannan's algorithm \cite{Kan83}, worst-case $2^{\mathcal{O}(n \log n)}$ => BKZ$_k$ outputs vector of length $\approx (k^{1/(2k)})^n\cdot \text{Vol}(\Lambda)^{1/n}$, for $n$-dimensional lattice \cite{{HS07, ABFKSW20} % TODO check if n or m

\cite{Che13}: $2^{1/(2e) k \log k + - 0.995 k + 16.25}$ => root Hermite factor of $\left(\frac{k}{2\pi e} \cdot (\pi k)^{1/k}\right)^{\frac{1}{2(k-1)}}$ 

\cite{ABFKSW20}: % TODO add to BKZ variants!!! => extended preprocessing and relaxing of search radius, 
% also add G6K (General Sieve Kernel) from ADHKPS19 (see more in sieving), running time of 2^Theta(k) but also 2^Theta(k) memory
achieve root Hermite factor $k^{\frac{1}{2k}(1 + o(1))}$ in $k^{k/8 + o(k)}$ time and polynomial memory, quantumly accelerated to $k^{k/16 + o(k)}$. Simulation data fit => $2^{0.125k \log k - 0.050k + 56}$
main idea - ``extended preprocessing'':  in preprocessing projected lattice of higher dimension $\lceil (1+c)\cdot k\rceil$ than rank $k$ is reduced => first Gram-Schmidt norms satisfy GSA, enumeration is faster on that part of the basis than on typical preprocessed BKZ block % TODO rewrite

\cite{ABLR21} 
- ``relaxing'' of the search radius with extreme pruning \cite{LN20} => exponential speed-up and ``extended preprocessing''  \cite{ABFKSW20}
- dual-hash technique for finding pairs for combination in enumeration

% TODO: for table
FastEnum & \cite{ABFKSW20} & $2^{0.125k \log k}$ => root Hermite factor of $k^(1/(2k))$
FastEnum + O(1)& \cite{ABFKSW20} & $2^{0.125k \log k - 0.050k + 56}$ => root Hermite factor of $k^(1/(2k))$
